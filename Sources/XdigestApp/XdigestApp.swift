...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// Sparkle updater controller. Owns the full update lifecycle:
    /// periodic checks, download, signature verification, in-place
    /// bundle replacement, and relaunch. `SUFeedURL` in Info.plist
    /// points at `appcast.xml` on `origin/main`; `release.sh` publishes
    /// new versions by pushing a new appcast entry.
    ///
    /// `startingUpdater: true` kicks off Sparkle's internal scheduler
    /// as soon as the lazy var is first touched (from `rebuildMenu`),
    /// which happens during `applicationDidFinishLaunching`.
    ...
    private func rebuildMenu() {
        ...
        // "Check for Updates..." routes directly to Sparkle's controller.
        // Sparkle owns its own NSMenuItem validation, download flow, and
        // progress UI -- we don't intermediate. Touching `updaterController`
        // here also lazy-instantiates it on first menu build, which
        // simultaneously kicks off Sparkle's periodic check scheduler.
